...
def _build_invite_message(
    ...
    html = (
        ...
        '<div style="background:linear-gradient(to right,#0D1B2A,#1a2a3a,#0D1B2A);padding:24px 20px;text-align:center">'
        '<div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:16px">'
        f'<span style="color:#ffffff80;font-size:12px;font-weight:600;letter-spacing:2px;text-transform:uppercase">JOIN</span>'
        '</div>'
        ...
        f'{host_section}'
        f'<img src="{settings.FRONTEND_URL}/logo-dark-trim.png" alt="accredit.vip" style="height:16px;width:auto;margin:12px 0;opacity:0.9" />'
...
def _build_qr_message(guest: Guest, event: Event, qr_image_url: str | None = None) -> tuple[str, str, str]:
    ...
    html = (
        ...
        f'<img src="{settings.FRONTEND_URL}/logo-dark-trim.png" alt="accredit.vip" style="height:14px;width:auto;margin:8px 0;opacity:0.9" />'
        '<p style="margin:0;letter-spacing:1px">Accredit.vip — Premium Event Infrastructure</p>'
        '</div>'
        '</div>'
    )
